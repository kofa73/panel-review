...
By default the seats get a **reference** to the diff (its canonical bytes on disk — an absolute
path plus size + sha256, not the body spliced into the prompt), a one-line scope label, and read
access to the tree — no statement of *intent*. You can supply that focus, which is shown to all three seats (blindness is
...
`debate.tmpl`, and `repair.tmpl` — the last re-asks a seat to fix only the JSON shape of its own
prior output, either a malformed block or a completed prose review it never fenced, using that
output plus the rejection reason). The emit schema for each block is single-sourced in
`prompts/schema/{findings,stances}.txt` and spliced into all three templates (so there is no schema
to drift). Each template also carries the **absolute review root** and directs a seat to set its
tool `cwd` to it — agy runs tools in its own sandbox and guesses the repo root, so a relative anchor
can drift; and the blind prompt references the diff **file** rather than inlining its body (a 240 KB
...
| `assemble` | Splice scope + a diff **reference** (or card paths) + the single-sourced emit schema into a prompt template without an LLM retyping them |
...
| `run_seat` | Dispatch one **CLI** seat (Codex or Gemini) for a tag, parse the block, and run the one-shot repair automatically; prints the final parse status. Repair fires on a **malformed** block (exit 5, any tag) or on a **no-fence** result (exit 4) for a required `findings`/`new_findings` block that is a **completed prose review** — gated to a seat that exited 0, whose output does not end with a wrapper-failure tail, and that carries enough prose (an error/timeout/refusal stub is left down, never pressured to fabricate). The repair **rebuilds** the raw rather than overwriting it, so a sibling `stances` block that `sweep` re-reads from the same file survives. The Claude seat is a subagent, not a CLI, so it is driven by the referee directly. **CLI seats are long-running** (minutes; `run_agy` up to ~34 min across its two 15-min model attempts), longer than the Bash tool's foreground timeout (2 min default, 10 min max), so the seats run **in the background**, never foreground (a foreground dispatch is killed mid-pass and the empty raw file reads as a down seat) |
